...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	    ...
	    StylesGenerator: __webpack_require__(3),
	    Color: __webpack_require__(4),
	    Colors: __webpack_require__(5),
	    LessSerializer: __webpack_require__(6),
	    MapStyleMixin: __webpack_require__(7),
	    RangeGenerator: __webpack_require__(8)
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	function ValueGenerator(obj) {
	    ...
	    this._core = obj._core || this._core || toTransformation(BezierEasing.linear);
	    return this;
	}
	...
	ValueGenerator.prototype = {
	    ...
	    setDomainTransformation: function setDomainTransformation(f) {
	        return this._setTransformation('_domain', f);
	    },

	    trim: function trim(trimFrom, trimTo) {
	        this._trimFrom = !!trimFrom;
	        this._trimTo = !!trimTo;
	        return this;
	    },

	    domain: function domain(from, to, transform) {
	        ...
	        return this.setDomainTransformation(function (val, args) {
	            ...
	            if (this._to === this._from) return 1;
	    ...
	    setRangeTransformation: function setRangeTransformation(f) {
	    ...
	    setCoreTransformation: function setCoreTransformation(f) {
	    ...
	    // ----------------------------------------------------------------------
	    //
	    _setTransformation: function _setTransformation(key, f) {
	        this[key] = f;
	        return this;
	    },

	    _wrapTransformation: function _wrapTransformation(key, f) {
...
/***/ function(module, exports) {
	...
	Color.prototype = {
	     ...
	     *            value in the range [0..1]
	     */
	    fadeout: function fadeout(amount) {
	        var hsl = this.toHSL();
	        hsl.a -= amount;
	        hsl.a = clamp(hsl.a);
	        return Color.fromHSL(hsl);
	    ...
	    contrast: function contrast(dark, light, threshold) {
	        // filter: contrast(3.2);
	        // should be kept as is, so check for color
	    ...
	    toARGB: function toARGB() {
	        return Color.toHex([this.rgba[3], this.rgba[0], this.rgba[1], this.rgba[2]]);
	    },
	    ...
	    compare: function compare(x) {
	        ...
	        return x.rgba[0] === this.rgba[0] && x.rgba[1] === this.rgba[1] && x.rgba[2] === this.rgba[2] && x.rgba[3] === this.rgba[3] ? 0 : -1;
	    },
	...
	Color.mix = function (first, second, weight) {
	    ...
	    var w1 = 1 - w2;
	    var rgba = [Math.round(first.rgba[0] * w1 + second.rgba[0] * w2), Math.round(first.rgba[1] * w1 + second.rgba[1] * w2), Math.round(first.rgba[2] * w1 + second.rgba[2] * w2), first.rgba[3] * p + second.rgba[3] * (1 - p)];
...
/***/ function(module, exports) {
	...
	LessSerializer.prototype = {
	    ...
	    serialize: function serialize(css) {
	        var result = '';
	        for (var key in css) {
	            var value = css[key];
	    ...
	    _serializeValues: function _serializeValues(shift, prefix, css) {
	        if (!prefix) prefix = '';
	        var result = '';
	        var that = this;
	        for (var key in css) {
	    ...
	    _serializeStyle: function _serializeStyle(name, css) {
	        var result = '';
	    ...
	    function generate(obj) {
	        ...
	        for (var key in obj) {
	            ...
	            if (typeof value === 'object') {
	                ...
	                if (typeof val === 'object' && typeof val.toStyle === 'function') {
	                    val = val.toStyle();
	                }
	                value = val;
	            }
